...
export class MainFormComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    // get All mainUI
    if (!this.mainUI$) {
      mainUIService.getCompanyMainUIs(this.companyID,
        this.modelMainUI.date).subscribe({
          next: (mainUI) => {
            if (mainUI) {
            ...
            this.loading.set(false); // Ensure loading is set to false on success
          },
          error: (error) => {
            console.error('Error fetching main UI data:', error);
  ...
  fetchMainUIData() {
    ...
    if (!this.apiCalled) {
      this.mainUIService.getCompanyMainUIs(this.companyID, this.modelMainUI.date).subscribe({
        next: (mainUI) => {
          console.log(mainUI);
          if (mainUI) {
            this.originalMainUI = mainUI;
      ...
      // If API was already called, still set loading to false
  ...
  onDateChange(): void {
    this.apiCalled = false; // Reset API called flag when date changes
    this.fetchMainUIData();
    this.searchMainUI = []; // Clear search results when date changes
    ...
    if (this.modelMainUI.pid && this.modelMainUI.name && this.modelMainUI.age && this.modelMainUI.pid != 'null' && this.modelMainUI.name != 'null' && this.modelMainUI.age != 'null') {
      ...
      if (this.editMainUIId) {
        ...
        this.editMainUISubscription = this.mainUIService
          ...
          .subscribe({
            next: () => {
              this.mainUIService.getCompanyMainUIs(
                this.companyID,
                this.modelMainUI.date
              ).subscribe((mainUI) => {
                if (mainUI) {
                  this.originalMainUI = mainUI;
                  this.mainUI = mainUI;
